...
export async function applyNanoBananaImagesToPaginas(
  ...
  async function generateCached(prompt: string): Promise<string | null> {
    ...
    // Prioridade: OpenRouter (funciona com OPENROUTER_API_KEY), Gemini direto como fallback
    let dataUrl: string | null = null;
    try {
      console.log(`[nano-banana] Tentando OpenRouter para: "${prompt.substring(0, 40)}..."`);
      dataUrl = await openRouterGenerateImage(prompt);
    } catch (e) {
      console.warn('[nano-banana] OpenRouter falhou:', e);
    }
    if (!dataUrl) {
      try {
        console.log(`[nano-banana] Tentando Gemini direto...`);
        dataUrl = await generateMaterialImage(prompt);
      } catch (e) {
    ...
    if (dataUrl) {
      ...
      console.log(`[nano-banana] ✅ Imagem gerada para: "${prompt.substring(0, 50)}..."`);
    } else {
