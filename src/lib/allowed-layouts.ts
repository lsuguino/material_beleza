...
/**
 * Troca layouts que exigem slots específicos (callouts no rodapé/topo) por layouts flexíveis
 * quando o conteúdo da página não preenche esses slots.
 *
 * - `A4_2_conteudo_misto` precisa de Dica (destaques[0]) + Exercício (destaques[1]). Se destaques
 *   está vazio ou só tem 1, a renderização cria callouts vazios → downgrade pra `A4_2_texto_corrido`.
 * - `A4_4_magazine` precisa de imagem OU CONCEITO-CHAVE (destaques[0]). Sem nada visual →
 *   downgrade pra `A4_2_texto_corrido`.
 */
export function downgradeLayoutsForContentFit(
  conteudo: Record<string, unknown>,
): Record<string, unknown> {
  const paginas = conteudo.paginas as Array<Record<string, unknown>> | undefined;
  if (!Array.isArray(paginas)) return conteudo;
  ...
  const fixed = paginas.map((p) => {
    ...
    const destaques = Array.isArray(p.destaques)
      ? (p.destaques as unknown[]).filter((d) => String(d).trim().length > 0)
      : [];
    const hasImage =
      Boolean(p.sugestao_imagem) || Boolean(p.imagem_url) || Boolean(p.prompt_imagem);

    if (layout === 'A4_2_conteudo_misto' && destaques.length < 1) {
      return { ...p, layout_tipo: 'A4_2_texto_corrido' };
    }

    if (layout === 'A4_4_magazine' && !hasImage && destaques.length === 0) {
      return { ...p, layout_tipo: 'A4_2_texto_corrido' };
    }
