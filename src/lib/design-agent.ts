...
CURSO VTSD (tema.id = "geral" OU nome contém "venda todo santo dia"): use estes layout_tipo para páginas de conteúdo, alternando ritmo visual.

TEMPLATES DE MIOLO (selecione o mais adequado ao conteúdo):
- A4_2_texto_corrido: texto longo, explicação detalhada — H1 + parágrafos + H3 intermediário
- A4_2_texto_citacao: texto reflexivo — header teal + corpo + citação em bloco com barra lateral
- A4_2_texto_imagem: conteúdo visual — título + parágrafo + imagem inline + legendas + parágrafos
- A4_2_texto_sidebar: leitura contínua com marcador — barra teal fina 60px + texto corrido
- A4_2_conteudo_misto: teoria + prática — dica do autor (teal) + corpo + exercício (teal)
- A4_2_duas_colunas: temas paralelos — título + 2 colunas iguais com subtítulos
- A4_2_duas_colunas_num: tópicos numerados — título + 2 colunas com números decorativos
- A4_4_magazine: dados visuais — título + imagem/gráfico + conceito-chave
- A4_4_cards_grid: pilares/princípios — grid 2×2 cards teal com número + título + descrição
- A4_4_destaque_numerico: estatísticas — grid 2×2 cards com números grandes (36px)
- A4_4_comparativo: certo vs errado — 2 colunas Evite (cinza) vs Faça (teal)
- A4_4_pros_contras: trade-offs — 2 colunas Vantagens (verde) vs Desafios (vermelho)
- A4_5_tabela: dados tabulares — tabela com header teal e linhas zebradas
- A4_5_organograma: hierarquia — 3 níveis com conectores visuais
- A4_5_mapa_mental: brainstorm — nó central + 4 ramos com sub-itens
- A4_5_timeline: cronograma — timeline vertical com indicadores circulares
- A4_5_infografico: funil/pipeline — barras decrescentes com labels
- A4_5_grafico_analise: análise — placeholder gráfico + insights numerados
- A4_6_faq: dúvidas — header teal + perguntas H3 + respostas body
- A4_6_lista_icones: ferramentas/recursos — ícone circular + título + descrição
- A4_6_texto_completo: leitura pura — H2 + parágrafos + H3 (sem visuais)
- A4_3_sidebar_steps: passo a passo — sidebar teal 370px + passos na coluna direita
- A4_3_processo_etapas: tutorial — sidebar teal + etapas numeradas com divisores
- A4_7_sidebar_conteudo: conteúdo rico — sidebar 225px + corpo + pullquote + steps + callout

TEMPLATES DE DESTAQUE (usar em momentos de pausa/impacto):
- A4_8_citacao_destaque: citação grande 36px centralizada em fundo teal
- A4_8_nota_importante: callout de alerta com ícone ⚠ em card teal
- A4_8_testemunho: depoimento com avatar + citação itálica 22px
- A4_8_frase_impacto: frase grande 48px em teal escuro sobre fundo branco
- A4_8_imagem_overlay: imagem full page + overlay teal no rodapé
- A4_8_imagem_sidebar: imagem 370px + sidebar teal com descrição

TEMPLATES DE ATIVIDADE (usar ao final de capítulos):
- A4_9_checklist: lista com checkboxes
- A4_9_exercicio: perguntas com área de escrita
- A4_9_resumo_capitulo: takeaways numerados com círculos teal
- A4_9_conceitos_chave: glossário com barra lateral + termo + definição

TEMPLATES DE ESTRUTURA:
- A4_0_sumario: índice com capítulos e páginas
- A4_1_abertura: header teal + imagem + texto intro
- A4_1_abertura_split: sidebar teal + conteúdo + número capítulo 96px
- A4_1_abertura_imagem: sidebar teal + imagem grande + texto
- A4_1_abertura_invertida: conteúdo esquerda + sidebar teal direita com tópicos
- A4_1_abertura_full: página inteira teal com título 48px centralizado
...
- NUNCA repita o mesmo layout_tipo em páginas consecutivas. Cada página DEVE ser visualmente diferente da anterior.
