...
/**
 * Padrões econômicos no OpenRouter (menor custo por token — reduzem erro de saldo em contas com pouco crédito).
 * Substitua por modelos premium em .env se preferir (OPENROUTER_MODEL_TEXT_MATERIAL / OPENROUTER_MODEL_DESIGN).
 */
export const OPENROUTER_MODELS = {
  textMaterial: 'openai/gpt-4o-mini',
  design: 'anthropic/claude-3.5-haiku',
} as const;

/** Usado quando o modelo configurado falha por saldo/créditos (402 ou mensagem do provedor). */
export const OPENROUTER_FALLBACK_CHAT_MODEL = 'openai/gpt-4o-mini';
 ...
 * Se o modelo configurado falhar por saldo insuficiente, tenta uma vez com modelo econômico (`OPENROUTER_FALLBACK_CHAT_MODEL`).
 */
export async function openRouterChat(params: OpenRouterChatParams): Promise<string> {
  ...
  try {
    ...
    const fallback = OPENROUTER_FALLBACK_CHAT_MODEL;
    if (
      primaryModel !== fallback &&
      looksLikeInsufficientCredits(status, detail)
    ) {
      console.warn(
        `[openrouter] Modelo "${primaryModel}" indisponível por saldo/custo; tentando "${fallback}".`
      );
...
export async function openRouterGenerateImage(prompt: string): Promise<string | null> {
  ...
  const didacticPrefix =
    'Fotografia profissional realista de alta qualidade para material didático impresso em A4. Estilo editorial moderno, iluminação natural, composição limpa sem texto sobreposto. Sem molduras, sem bordas, imagem sangrada que preenche todo o quadro. ';
