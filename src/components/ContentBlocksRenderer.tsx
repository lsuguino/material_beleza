 ...
 * - text → <p>
 * - image → <img> só com URL renderizável; sem imagem → não ocupa espaço
...
export function ContentBlocksRenderer({ blocks, className = '' }: ContentBlocksRendererProps) {
  ...
  return (
    <div className={['[&_p]:text-justify [&_p]:hyphens-auto', className].filter(Boolean).join(' ')}>
      {blocks.map((block, i) => {
        if (block.type === 'text') {
        ...
        if (block.type === 'image') {
          ...
          if (!isRenderableImageUrl(src)) return null;
          return (
            <figure key={i} className="my-4 w-full overflow-hidden rounded-sm flex justify-center">
              <img
                src={src}
                alt=""
