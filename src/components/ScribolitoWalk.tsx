...
export function ScribolitoWalk({
  ...
  // Inicia com índice fixo (0) para evitar hydration mismatch, randomiza no cliente
  const [phraseIndex, setPhraseIndex] = useState(0);
  useEffect(() => {
    setPhraseIndex(Math.floor(Math.random() * SCRIBOLITO_PHRASES.length));
  ...
  const displayBalloon = hideBalloon ? '' : (balloon || SCRIBOLITO_PHRASES[phraseIndex]);
