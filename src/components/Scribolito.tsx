...
export type ScribolitoFace = 'relaxado' | 'sorrindo' | 'apaixonado' | 'surpreso';

const FACE_BY_NAME: Record<ScribolitoFace, string> = {
  relaxado: '/scribolito/faces/relaxado.png',
  sorrindo: '/scribolito/faces/sorrindo.png',
  apaixonado: '/scribolito/faces/apaixonado.png',
  surpreso: '/scribolito/faces/surpreso.png',
};
...
interface ScribolitoProps {
  ...
  walking?: boolean;
  /** Rosto fixo — quando definido, trava a animação de troca de expressão. */
  face?: ScribolitoFace;
  /** CSS class adicional */
  className?: string;
}
...
export function Scribolito({
  ...
  // Face change animation — skip if a face is pinned
  useEffect(() => {
    if (!imagesLoaded || face) return;
    const interval = setInterval(() => {
      setFaceFrame(prev => (prev + 1) % FACE_FRAMES.length);
    }, faceChangeInterval);
    return () => clearInterval(interval);
  }, [faceChangeInterval, imagesLoaded, face]);
  ...
  return (
    <div className={`relative ${className}`} style={{ width: size, height: size }}>
      ...
      <img
        src={face ? FACE_BY_NAME[face] : FACE_FRAMES[faceFrame]}
        alt=""
