...
export default function Home() {
  ...
  const openPreviewFull = useCallback(async () => {
    if (generatedData && typeof window !== 'undefined') {
      await savePreviewDataToClient(generatedData);
    }
    if (typeof window !== 'undefined') {
  ...
  const handleDownloadPdfMain = useCallback(async () => {
    ...
    try {
      ...
      if (pdfId) {
        ...
        try {
          blob = await fetchPdfBlobFromPreviewData(previewUrl, generatedData);
        } catch (fetchErr) {
          if (fetchErr instanceof Error && fetchErr.message.includes('Falha')) throw fetchErr;
          throw new Error(
            fetchErr instanceof Error
              ? fetchErr.message
              : 'Falha de conexão ao gerar PDF. Verifique se o servidor está rodando.'
          );
        }
      }
  ...
  return (
    <div className="font-body bg-background text-on-surface flex min-h-screen w-full flex-col overflow-x-hidden selection:bg-primary/20 dark:selection:bg-primary/35 dark:selection:text-white">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden lg:flex-row">
        {/* Sidebar colapsável */}
        <aside className={`order-2 flex shrink-0 flex-col border-t border-outline-variant/20 bg-surface font-body text-sm font-medium dark:bg-background-dark/30 lg:order-1 lg:h-auto lg:max-h-none lg:border-r lg:border-t-0 lg:overflow-y-auto transition-all duration-300 ${
          sidebarCollapsed ? 'lg:w-16' : 'lg:w-80'
        } w-full`}>
          {/* Controles da sidebar (logo na barra global) */}
          <div className={`flex items-center border-b border-outline-variant/15 dark:border-zinc-700/40 ${sidebarCollapsed ? 'flex-col gap-3 px-2 py-4' : 'justify-end px-5 py-4'}`}>
            {sidebarCollapsed ? (
              <>
                <button type="button" onClick={() => setSidebarCollapsed(false)} title="Expandir" className="p-1.5 rounded-lg text-on-surface-variant hover:text-primary hover:bg-surface-container dark:hover:bg-zinc-700/60 transition-colors">
                  <span className="material-symbols-outlined text-[20px]">menu</span>
                </button>
              </>
            ) : (
              <>
                <button type="button" onClick={() => setSidebarCollapsed(true)} title="Recolher" className="p-1.5 rounded-lg text-on-surface-variant hover:text-primary dark:text-white/50 hover:bg-surface-container dark:hover:bg-zinc-700/60 transition-all">
          ...
          {/* Conteúdo colapsado: só logo do curso selecionado */}
          {sidebarCollapsed ? (
            <div className="flex-1 flex flex-col items-center gap-4 py-6">
              {(() => { const opt = COURSE_PICKER_OPTIONS.find(o => o.id === cursoId); return opt?.logoSrc ? (
                <img
                  src={opt.logoSrc}
                  alt={opt.ariaLabel}
                  className="h-auto w-full max-h-10 max-w-[7rem] object-contain brightness-0 dark:brightness-100 opacity-80"
                />
              ) : null; })()}
            </div>
          ) : (
          ...
          <div className="px-5 pt-5 pb-3">
            <h2 className="font-headline text-base font-bold tracking-tight text-on-surface dark:text-white">Configurações do material</h2>
          </div>
                    ...
                    >
                      {opt.logoSrc ? (
                        <img
                          ...
                          alt=""
                          draggable={false}
                          className={`h-auto w-full min-h-0 max-h-6 max-w-full object-contain object-center sm:max-h-7 ${
            ...
            <div className="px-6">
              <div className="flex items-center justify-between gap-3 text-on-surface-variant dark:text-white/65">
                <div className="flex min-w-0 items-center gap-2">
                  <span className="material-symbols-outlined shrink-0 text-[20px]">psychology</span>
              ...
              <p className="mt-2 text-[11px] leading-relaxed text-on-surface-variant/90 dark:text-white/45">
                Adiciona uma página de atividades no final do material (perguntas sobre o conteúdo e linhas para responder), em material completo ou resumo. Com pasta completa, vale para o primeiro arquivo do lote quando esta opção estiver ativa.
              </p>
            </div>
          </nav>
          </>
        ...
        {/* Área principal */}
        <main className="relative order-1 flex min-h-0 min-w-0 flex-1 flex-col overflow-y-auto bg-background lg:order-2">
          <div className="pointer-events-none absolute right-0 top-0 -mr-48 -mt-48 h-[min(600px,80vw)] w-[min(600px,80vw)] rounded-full bg-primary/5 blur-[120px] dark:bg-primary/10" />
          <div className="relative z-10 mx-auto w-full max-w-6xl px-4 py-3 sm:px-6 lg:px-10 lg:py-6">
            <FooterParticles hasFile={!!file} generating={loading} />

            <div className="mb-3 max-w-3xl lg:mb-5 xl:mb-6">
              <h1 className="text-2xl font-bold leading-[1.1] tracking-tight text-on-surface dark:text-white sm:text-3xl lg:text-4xl xl:text-5xl 2xl:text-6xl" style={{ fontFamily: "'Transforma Sans', sans-serif" }}>
              ...
              <p className="mt-2 max-w-xl text-xs font-medium leading-relaxed text-on-surface-variant dark:text-white/55 sm:text-sm lg:text-base xl:text-lg">
                Envie transcrições, PDFs ou textos e gere materiais de estudo com diagramação profissional, alinhados ao curso selecionado.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 items-start gap-4 lg:grid-cols-12 lg:gap-6">
              ...
              <div className={loading ? 'lg:col-span-8' : 'lg:col-span-8'}>
                {loading ? (
                  <div
                    ref={workspaceRef}
                    className="generation-workspace-breath relative flex min-h-[320px] lg:min-h-[400px] xl:min-h-[480px] w-full flex-col overflow-hidden rounded-xl border-2 border-dashed border-primary/35 bg-surface-container-lowest shadow-2xl shadow-on-surface/5 dark:border-primary/25 dark:bg-surface-container-low/90 dark:ring-1 dark:ring-outline-variant/25"
                  >
                    ...
                    return (
                      <div className="rounded-xl border border-outline-variant/30 bg-surface-container-low shadow-md dark:border-outline-variant/35 dark:bg-surface-container/60 overflow-hidden">
                        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-outline-variant/20 px-4 py-3 dark:border-outline-variant/30">
                          ...
                          <div className="flex flex-wrap items-center gap-1.5">
                            ...
                            >
                              ...
                              {pdfLoading ? 'Aguardando...' : pdfReady ? '⚡ PDF' : 'PDF'}
                            </button>
                            <button
                              type="button"
                        ...
                        {generatedData.perguntas && generatedData.perguntas.length > 0 && (
                          <div className="border-t border-outline-variant/20 bg-primary/5 px-4 py-3 dark:border-outline-variant/30 dark:bg-primary/10">
                            <p className="mb-2 text-xs font-bold uppercase tracking-wider text-primary">Perguntas para reflexão</p>
                            <ol className="list-inside list-decimal space-y-1.5 text-sm leading-snug text-on-surface dark:text-white/90">
                  ...
                  <div className="group w-full">
                    ...
                    <div
                      ref={workspaceRef}
                      className={`relative overflow-hidden rounded-xl border-2 border-dashed bg-surface-container-lowest shadow-2xl shadow-on-surface/5 dark:bg-surface-container-low/90 dark:ring-1 dark:ring-outline-variant/25 ${dropzoneClass}`}
                    >
                      ...
                      <label
                        ...
                        onDrop={handleDrop}
                        className="relative z-10 flex min-h-[280px] cursor-pointer flex-col items-center justify-center p-8 text-center sm:min-h-[360px] lg:min-h-[280px] lg:min-h-[360px]"
                      >
                        <div className="flex w-full max-w-xl flex-col items-center justify-center px-3 sm:px-4">
                          {file ? (
                            ...
                            <>
                              <p className="font-headline mb-4 text-center text-lg font-semibold leading-snug text-on-surface dark:text-white sm:text-xl sm:leading-relaxed">
                                Solte aqui a transcrição da aula ou qualquer arquivo de texto, ou selecione-o no seu dispositivo.
                              </p>
                              <p className="mb-8 text-center text-xs leading-relaxed text-on-surface-variant dark:text-white/70 sm:text-sm">
                                Formatos suportados: PDF, TXT, VTT, SRT, MD, CSV, JSON, XML e demais arquivos de texto (até 50&nbsp;MB).
                              </p>
                            </>
                          )}
                ...
                {loading ? (
                  ...
                  <>
                    ...
                    <div className="flex justify-center">
                      <div className="hidden 2xl:block"><ScribolitoWalk size={320} speed="slow" shadow /></div>
                      <div className="hidden xl:block 2xl:hidden"><ScribolitoWalk size={250} speed="slow" shadow /></div>
                      <div className="hidden lg:block xl:hidden"><ScribolitoWalk size={180} speed="slow" shadow /></div>
                      <div className="hidden sm:block lg:hidden"><ScribolitoWalk size={140} speed="slow" shadow /></div>
                      <div className="block sm:hidden"><ScribolitoWalk size={110} speed="slow" shadow /></div>
                    </div>
                  </>
                )}
              </div>
              ...
              <footer className="relative z-10 mt-auto pt-6 pb-4 text-center overflow-hidden">
                <span className="inline-block scale-[2] origin-center">
