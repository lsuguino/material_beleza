...
Responde APENAS com um JSON válido (sem markdown, sem \`\`\`) no formato:
{
  ...
  "sections": [
    {
      "title": "Mapa em balões",
      "blocks": [
        {
          "type": "mind_map",
          "center": "string - tema central da aula (2 a 5 palavras)",
          "items": ["ponto chave 1", "ponto chave 2", "ponto chave 3", ...],
...
REGRAS para o mapa em balões:
- Extraia da transcrição APENAS os PONTOS CHAVE da aula: conceitos principais, termos essenciais, conclusões importantes.
- "items" = 4 a 10 pontos chave, cada um em 2 a 6 palavras. Nada inventado — só o que foi efetivamente abordado na aula.
- O resultado será exibido como balões conectados: um balão central (center) ligado por linhas a cada balão de ponto chave (items).
`;
...
${materialSchema}`;
    ...
    const systemPromptMindmap = `Você é um expert em síntese. Use SOMENTE o conteúdo da transcrição do .vtt — sem pesquisa externa. Analise a transcrição e extraia APENAS os PONTOS CHAVE da aula para um mapa em balões: um tema central (center) e de 4 a 10 itens, cada item sendo um ponto chave realmente mencionado (conceito, termo ou conclusão importante). Frases curtas (2 a 6 palavras por item). Nada inventado.
${mindmapSchema}`;
    ...
    const parsed = JSON.parse(jsonStr) as Omit<TeachingMaterial, 'createdAt'>;
