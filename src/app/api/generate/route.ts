...
function applyDefaultDesign(
  ...
  // Layouts com renderizadores implementados (base + novos do Figma)
  const VTSD_BASE_LAYOUTS = [
    'A4_1_abertura',
    'A4_2_conteudo_misto',
    'A4_2_texto_corrido',
    'A4_2_texto_citacao',
...
function alternateVtsdContentLayouts(conteudo: Record<string, unknown>): Record<string, unknown> {
  ...
  const alternadas = paginas.map((p) => {
    ...
    // Se o design-agent já atribuiu um layout válido dos base, manter
    const BASE_SET = new Set(['A4_1_abertura', 'A4_2_conteudo_misto', 'A4_3_sidebar_steps', 'A4_4_magazine', 'A4_7_sidebar_conteudo']);
    ...
    if (bestLayout === lastLayout) {
      const alternatives = ['A4_2_conteudo_misto', 'A4_3_sidebar_steps', 'A4_4_magazine', 'A4_7_sidebar_conteudo', 'A4_1_abertura'];
      bestLayout = alternatives.find(l => l !== lastLayout) || 'A4_2_conteudo_misto';
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fix 4: Validar que TODAS as páginas de conteúdo têm layout_tipo válido
    const validarLayouts = (obj: Record<string, unknown>): Record<string, unknown> => {
      const paginas = obj.paginas as Array<Record<string, unknown>> | undefined;
      if (!Array.isArray(paginas)) return obj;
      const VALID_BASE = new Set(['A4_1_abertura', 'A4_2_conteudo_misto', 'A4_3_sidebar_steps', 'A4_4_magazine', 'A4_7_sidebar_conteudo']);
      let idx = 0;
      const BASE_SEQ = ['A4_1_abertura', 'A4_2_conteudo_misto', 'A4_3_sidebar_steps', 'A4_4_magazine', 'A4_7_sidebar_conteudo'];
      const fixed = paginas.map((p) => {
        if ((p.tipo as string) !== 'conteudo') return p;
        const lt = (p.layout_tipo as string) || '';
        idx++;
        // Se não tem layout ou não começa com A4_, atribuir um dos base
        if (!lt || (!lt.startsWith('A4_') && cursoId === 'geral')) {
          return { ...p, layout_tipo: BASE_SEQ[(idx - 1) % BASE_SEQ.length] };
        }
        return p;
      });
      return { ...obj, paginas: fixed };
    };

    const conteudoValidado = validarLayouts(conteudoComDesign);
    ...
    const withAtividades =
      perguntas && perguntas.length > 0 ? injectAtividadesFinaisPage(normalized, perguntas) : normalized;
    const finalNorm =
