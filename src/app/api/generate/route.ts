...
import { sanitizeConteudoLayouts, VTSD_ALTERNATION_POOL, downgradeLayoutsForContentFit } from '@/lib/allowed-layouts';
 ...
 */
/**
 * Alterna layouts garantindo variação visual real.
 ...
 *
 * Funciona pra TODOS os cursos (não só VTSD). A regra de "primeira página =
 * A4_1_abertura" foi removida porque `injectTopicOpeners` já faz isso
 * deterministicamente entre mudanças de titulo_bloco.
 */
function alternateContentLayouts(conteudo: Record<string, unknown>): Record<string, unknown> {
  const paginas = conteudo.paginas as Array<Record<string, unknown>> | undefined;
  if (!Array.isArray(paginas) || paginas.length === 0) return conteudo;

  // Pool de layouts base — embaralhados a cada geração para variar o padrão.
  ...
  const alternadas = paginas.map((p) => {
    ...
    // Openers injetados preservam seu layout (A4_1_abertura) — não alterar.
    if (p._isTopicOpener) return p;
    // Páginas de continuação idem — layout é forçado pelo paginador.
    if (p.continuacao) return p;

    const currentLayout = (p.layout_tipo as string) || '';

    // Se o design-agent já atribuiu um layout válido do pool e não repete, manter.
    ...
    const destCount = Array.isArray(p.destaques) ? (p.destaques as string[]).length : 0;
    const blocoLen = String(p.bloco_principal ?? '').trim().length;
    const hasPracticalDensity = itensCount >= 4 || hasTabela || hasFluxograma;
    const isShortBody = blocoLen > 0 && blocoLen < 150;
    const isPureCitation = hasCitacao && !hasItens && !hasDestaques && blocoLen < 220;
    const isImpactOnly = isShortBody && (hasDestaques || hasCitacao) && !hasItens && !hasGrafico && !hasImagem;

    /**
     * Pré-cálculo de "candidatos por sinal" — usado tanto pra escolha principal
     * quanto pro fallback anti-repetição (pra não cair em layout qualquer).
     */
    const candidatosPorSinal = (): string[] => {
      if (hasGrafico || hasImagem) return ['A4_4_magazine'];
      if (isPureCitation) return ['A4_8_citacao_destaque', 'A4_2_texto_citacao'];
      if (isImpactOnly) return ['A4_8_frase_impacto', 'A4_8_citacao_destaque'];
      if (hasPracticalDensity) return ['A4_3_sidebar_steps', 'A4_7_sidebar_conteudo'];
      if (hasItens && itensCount >= 2 && hasDestaques) return ['A4_2_conteudo_misto', 'A4_7_sidebar_conteudo'];
      if (hasDestaques && hasCitacao) return ['A4_2_conteudo_misto', 'A4_2_texto_citacao'];
      if (hasCitacao) return ['A4_2_texto_citacao', 'A4_7_sidebar_conteudo'];
      if (hasItens) return ['A4_7_sidebar_conteudo', 'A4_2_texto_sidebar'];
      if (destCount >= 2) return ['A4_2_conteudo_misto'];
      return ['A4_2_texto_corrido', 'A4_2_texto_sidebar', 'A4_7_sidebar_conteudo'];
    };

    const candidatos = candidatosPorSinal();
    // 1ª escolha: primeiro candidato diferente do último layout.
    let bestLayout = candidatos.find((c) => c !== lastLayout) ?? candidatos[0];

    // Se todos os candidatos colidem com lastLayout (caso raro), cai pro pool embaralhado.
    if (bestLayout === lastLayout) {
      const fromSequence = LAYOUT_SEQUENCE.find((l) => l !== lastLayout);
      bestLayout = fromSequence ?? 'A4_2_conteudo_misto';
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      // 1a. Pré-scan visual (Haiku) — detecta trechos que viram gráfico/fluxograma/tabela.
      // Só roda nos caminhos que vão passar por generateContent (não em design_only/resumido parser).
      const deveEscanearHints = modo !== 'design_only' && !(isTextoOrganizado && modo === 'resumido');
      const hints = deveEscanearHints ? await scanVisualHints(transcricao) : [];
      ...
      if (modo === 'design_only') {
        ...
        if (modo === 'resumido') {
          ...
          conteudo = (await generateContent(transcricao, 'completo', nomeCurso, hints)) as unknown as Record<string, unknown>;
        }
      } else {
        conteudo = (await generateContent(transcricao, modo, nomeCurso, hints)) as unknown as Record<string, unknown>;
      }
    } catch (contentErr) {
    ...
    /** Rotação determinística de layouts — agora roda pra todos os cursos (Fase 3). */
    const conteudoComLayoutAlternado = alternateContentLayouts(
      conteudoValidado as Record<string, unknown>,
    );

    /** Downgrade de layouts que exigem callouts/imagem quando a página não tem o conteúdo pra preencher. */
    const conteudoComLayoutCompativel = downgradeLayoutsForContentFit(conteudoComLayoutAlternado);

    const previewBaseUrl = request.nextUrl?.origin;
    const conteudoPaginado = await paginateWithValidation(
